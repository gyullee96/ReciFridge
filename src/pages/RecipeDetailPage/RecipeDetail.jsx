...
const RecipeDetail = () => {
  const { id } = useParams();
  const { data, isLoading, isError, error } = useRecipeDetail({
    id,
    keyword: 'test',
    page: 1,
    filter: null,
  ...
  return (
    <div className="recipe-main">
      ...
      <h1 style={{ color: 'black', fontWeight: 700 }}>{recipe.RCP_NM}</h1>
      <div style={{ border: 'none', borderTop: '0.5rem solid #A1C8C4' }} />

      <p style={{ width: '80%', textAlign: 'center' }}>{recipe.RCP_NA_TIP}</p>
      <div style={{ border: 'none', borderTop: '0.5rem solid #A1C8C4' }} />
